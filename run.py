...
class anaxcelhandler(QtWidgets.QMainWindow, UI_lan.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.statusbar.showMessage('论文评阅答辩分配')
    ...
    def confirm_idx(self):
        ...
        if wb == '' or ws == '':
            QMessageBox.about(self, "您好", '请先load文件')
        else:
            x = int(x) if x != '' else x
            y = int(y) if y != '' else y
    ...
    def SplitProcess(self):
        if not self.flag_confirm:
            QMessageBox.about(self, "您好", '请先选择文件并load文件,并选择拆分关键词')
        else:
            try:
                ...
                QMessageBox.about(self, "您好", '拆分{}出现错误'.format(base_name))
        self.flag_confirm = False
    def mergeProcess(self):
        QMessageBox.about(self, "您好", '此功能为付费功能')
