...
def output(group_day1, group_day2):
    ...
    worksheet = workBook.add_sheet("预答辩")

    style_title = style_head()
    style_cont = style_content()

    for j in range(5):
        worksheet.write_merge(0, 0, j * 5, j * 5 + 4, '预答辩', style_title)
        worksheet.write_merge(1, 1, j * 5, j * 5 + 4, f'第1日 第{j + 1}组', style_title)
        worksheet.write_merge(2, 2, j * 5, j * 5 + 4, f'预答辩委员：{group_day1[j].group_leader.tutorName}（组长）'
                                                      f'{group_day1[j].get_members_str()}（结果反馈）', style_title)
        worksheet.write_merge(3, 3, j * 5, j * 5 + 4, f'预答辩地点：', style_title)
    ...
    for j in range(5):
        worksheet.write_merge(20, 20, j * 5, j * 5 + 4, '预答辩', style_title)
        worksheet.write_merge(21, 21, j * 5, j * 5 + 4, f'第2日 第{j + 1}组', style_title)
        worksheet.write_merge(22, 22, j * 5, j * 5 + 4, f'预答辩委员：{group_day2[j].group_leader.tutorName}（组长）'
                                                        f'{group_day2[j].get_members_str()}（结果反馈）', style_title)
        worksheet.write_merge(23, 23, j * 5, j * 5 + 4, f'预答辩地点：', style_title)
